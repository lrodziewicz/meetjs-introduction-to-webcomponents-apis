/*! relayr-browser-sdk 2015-02-09 */
window.RELAYR=function(a,b){function c(a){function c(b){function c(){i.user().getUserInfo(d,function(){b.success(d)},function(a){b&&b.error&&"function"==typeof b.error?b.error(a):(localStorage.removeItem("relayrToken"),i.login(b))})}if(!a||!e.nonEmptyString(a.redirectUri))throw new Error("Provide credentials: redirectUri");if(!b||!b.success||"function"!=typeof b.success)throw new Error("Provide the method success within your parameters");var d=localStorage.getItem("relayrToken");if(d)c();else{var f={client_id:i.credentials.appId,redirect_uri:i.credentials.redirectUri,scope:"access-own-user-info"};this.login.redirect(h.url+"oauth2/auth?client_id="+f.client_id+"&redirect_uri="+f.redirect_uri+"&response_type=token&scope="+f.scope)}}function d(){this.getUserInfo=function(a,b,c){if(i.account){if(!b||"function"!=typeof b)return i.account;b(i.account)}else e.ajax({url:i.config.url+"oauth2/user-info",type:"GET",token:"Bearer "+a,isObject:!0},function(c){c.token=a,i.account=c,b(i.account)},function(a){c(a)});return this},this.logout=function(){localStorage.removeItem("relayrToken")}}function f(){this.getDeviceData=function(b,c,d){if(!b||!b.incomingData||"function"!=typeof b.incomingData)throw new Error("Provide the method incomingData within your parameters");if(!b||!b.deviceId)throw new Error("Provide the deviceId within your parameters");return b.token||i.account.token&&(b.token=i.account.token),b.token=b.token.replace("Bearer ",""),e.ajax({url:i.config.url+"apps/"+a.appId+"/devices/"+b.deviceId,type:"POST",token:"Bearer "+b.token,isObject:!0},function(a){var c=PUBNUB.init({ssl:!0,subscribe_key:a.subscribeKey,cipher_key:a.cipherKey,auth_key:a.authKey});c.subscribe({channel:a.channel,message:function(a){b.incomingData&&b.incomingData(JSON.parse(a))},error:function(a){b.error&&b.error(a)}})},function(a){if(!b||!b.error||"function"!=typeof b.error)throw new Error("Provide the method error within your parameters");d(a),localStorage.removeItem("relayrToken"),i.login()}),this},this.getAllDevices=function(a,b){return e.ajax({url:i.config.url+"users/"+i.account.id+"/devices",type:"GET",token:"Bearer "+i.account.token,isObject:!0},function(b){a(b)},function(){if(!b||!b||"function"!=typeof b)throw new Error("Provide the method error within your parameters")}),this}}function g(){i.checkAccount(),this.getTransmitters=function(a,b){return e.ajax({url:i.config.url+"users/"+i.account.id+"/transmitters",type:"GET",token:"Bearer "+i.account.token,isObject:!0},function(b){a(b)},function(){if(!b||!b||"function"!=typeof b)throw new Error("Provide the method error within your parameters")}),this}}if(!a||!e.nonEmptyString(a.appId))throw new Error("Provide credentials: appId and redirectUri");var h={url:"https://api.relayr.io/",mockMode:!1},i=this;i.credentials=a,i.config=h,i.checkAccount=function(){if(!i.account)throw new Error("You must be logged in to access this method.")},function(){function a(a){var b=a.split("#");if(b.length<2)return void 0;var c=b[1].split("&"),d=c.reduce(function(a,b){var c=b.split("=");return a[c[0]]=c[1],a},{});return d.access_token}var b=a(document.location.hash),c=window.location.href,d=c.indexOf("#access_token");d>0&&(window.location=c.substring(0,d)),b&&localStorage.setItem("relayrToken",b)}(),this.login=c,this.user=function(){return new d},this.devices=function(){return new f},this.transmitters=function(){return new g},this.login.redirect=function(a){b.location=a}}var d={init:function(a){return new c(a)}},e={nonEmptyString:function(a){return a&&"string"==typeof a&&a.length>0},ajax:function(a,b,c){var d=new XMLHttpRequest;d.onreadystatechange=function(){4===d.readyState&&(d.status>199&&d.status<299&&b(a.isObject?JSON.parse(d.responseText):d.responseText),401===d.status&&c(d))},d.open(a.type,a.url,!0),d.setRequestHeader("Authorization",a.token),d.send()},loadScript:function(a,b){var c=document.getElementsByTagName("head")[0],d=document.createElement("script");d.type="text/javascript",d.src=a,d.onreadystatechange=b,d.onload=b,c.appendChild(d)}};return d}(window,window.document);
// Version: 3.7.8
(function(){
